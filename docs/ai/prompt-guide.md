...
每次请求会附带本次发送时间 `sent_at`。模型解析时间时遵循：

1. 文字或图片凭证中有明确日期/时间时，以凭证为准。
2. 只有“今天、现在、刚刚”等相对表达时，按 `sent_at` 解释。
3. 没有任何时间线索时，一般按发送当天当时返回 `timestamp`。

模型的 `reply_text` 只能说明“已整理为待确认卡片”，不得说“已记录、已保存、已入账、成功记账”，因为正式入账必须由用户点击确认保存。
