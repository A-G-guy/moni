...
| `dailyRemaining` | 当前月设置月总预算时，表示 `(月总预算 - 本月总支出) / 今日之后剩余天数`；提前记账的未来支出也会占用预算，因此可用 `todayExpense <= dailyRemaining` 判断当前节奏是否会导致月总预算超限。若今天是月末，则退化为剩余预算。未设置月总预算时，保留 `月结余 / 剩余天数` 的退化口径。 |
