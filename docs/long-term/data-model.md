...
| `sort_order` | `i32` | NOT NULL, DEFAULT `0` | 排序权重，数值越小越靠前 |
| `parent_id` | `Option<i64>` | FK → `categories(id)`, NULL | 父分类 ID，NULL 表示一级分类，非 NULL 表示二级分类 |
| `is_preset` | `bool` | NOT NULL, DEFAULT `0` | **已废弃**：预设分类概念已移除，所有分类一视同仁，保留字段兼容旧数据 |
...
**当前版本**：`CURRENT_SCHEMA_VERSION = 1`（定义于 `moni-core/src/db/schema.rs`），用于 ZIP 备份 manifest 的版本标记。

**原则**：
- 新表用 `CREATE TABLE IF NOT EXISTS`，新索引用 `CREATE INDEX IF NOT EXISTS`。
- 新增列先通过 `pragma_table_info` 检测存在性，避免重复 `ALTER` 报错。
- 不删除旧列（如 `color_hex`、`is_preset`），保持向后兼容。
...
Rust DTO 经 `serde(rename_all = "camelCase")` 序列化为 JSON 后，由 Kotlin 的 `kotlinx.serialization` 反序列化为对应数据类。字段对应关系如下：

### CoreCategory（Kotlin）← CategoryDto（Rust）

| Kotlin 字段 | 类型 | Rust 字段 | 说明 |
|-------------|------|-----------|------|
| `id` | `Long` | `id` | |
| `name` | `String` | `name` | |
| `description` | `String?` | `description` | 可空，默认 `null` |
| `categoryType` | `String` | `category_type` | `"income"` / `"expense"` |
| `iconName` | `String` | `icon_name` | |
| `sortOrder` | `Int` | `sort_order` | |
| `parentId` | `Long?` | `parent_id` | 可空，默认 `null` |
| `archivedAt` | `Long?` | `archived_at` | 可空，默认 `null` |
| `createdAt` | `Long` | `created_at` | 默认 `0` |
| `updatedAt` | `Long` | `updated_at` | 默认 `0` |

**注意**：`color_hex` 与 `is_preset` 已废弃，不在 DTO 中暴露；Kotlin 侧不感知这两个字段。

### CoreRecord（Kotlin）← RecordDto（Rust）

| Kotlin 字段 | 类型 | Rust 字段 | 说明 |
|-------------|------|-----------|------|
| `id` | `Long` | `id` | |
| `amountCents` | `Long` | `amount_cents` | |
| `recordType` | `String` | `record_type` | `"income"` / `"expense"` |
| `categoryId` | `Long` | `category_id` | |
| `categoryName` | `String` | `category_name` | 由 Rust 侧展开关联查询填充 |
| `note` | `String` | `note` | |
...
| 2026-05-05 | 去预设：彻底移除预设分类概念 | `is_preset` 字段从实体/DTO 中移除，数据库保留兼容 |
