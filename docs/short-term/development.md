...
| Rust toolchain | stable（项目自动激活） | 由 `rust-toolchain.toml` 控制，当前为 1.93.0 |
...
cargo llvm-cov --workspace --summary-only  # Rust 覆盖率速览（需安装 cargo-llvm-cov）
...
**IDE 设置冲突解决**：
- **IntelliJ IDEA / RustRover**：`Settings → Languages & Frameworks → Rust → Rustfmt`，勾选 "Use rustfmt instead of built-in formatter"，并在 `Additional arguments` 中留空（让 rustfmt 自动读取 `rustfmt.toml`）。
- **VSCode**：`settings.json` 中添加 `"rust-analyzer.rustfmt.extraArgs": []`，确保不传入与项目冲突的参数。
...
### 6.6 cargo test 在 worktree 中找不到测试
若从 git worktree 运行 `cargo test --workspace`，可能因 `Cargo.lock` 路径解析或 target 目录共享导致异常。建议：
- 确保 worktree 内 `Cargo.lock` 与主仓库一致（worktree 默认共享 `.git`，但工作目录独立）。
- 若遇到路径相关错误，尝试在 worktree 内先执行 `cargo check --workspace` 建立本地 target 缓存后再跑测试。
- 极端情况下，可设置环境变量 `CARGO_TARGET_DIR=target_worktree` 隔离 target 目录。
...
- ktlint-wrapper 同版本
- Rust 1.93.0 stable（workspace lints 已稳定）
