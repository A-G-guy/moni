...
fn verify_file_checksum<R: Read + std::io::Seek>(
    ...
    let mut entry = zip
        ...
        .map_err(|e| crate::core::error::CoreError::BackupCorrupted(format!(
...
/// 执行全量恢复。
/// `pre_restore_snapshot_path` 为可选的恢复前快照路径，恢复失败时从此快照回滚。
pub fn backup_restore(
    ...
    if let Some(cb) = on_progress { cb("打开备份文件...", 5); }
    let file = File::open(in_zip_path)
        .map_err(|e| crate::core::error::CoreError::BackupIo(format!(
            "打开备份 ZIP 失败: {e}")))?;
    let mut zip = zip::ZipArchive::new(file)
        .map_err(|e| crate::core::error::CoreError::BackupZipError(e.to_string()))?;

    // 1. 读取并校验 manifest
    if let Some(cb) = on_progress { cb("读取清单...", 15); }
    let manifest = read_manifest(&mut zip)?;
    validate_format_version(manifest.format_version)?;
    verify_manifest_integrity(&manifest)?;

    // 2. 逐文件 SHA-256 校验
    if let Some(cb) = on_progress { cb("校验文件完整性...", 30); }
    ...
    // 3. 解压数据库到临时位置
    if let Some(cb) = on_progress { cb("解压数据库...", 50); }
    ...
    // 4. 打开临时数据库，运行迁移
    if let Some(cb) = on_progress { cb("迁移数据...", 65); }
    let tmp_conn = Connection::open(&tmp_db)
        .map_err(|e| crate::core::error::CoreError::Database(format!(
            "打开恢复数据库失败: {e}")))?;
    schema::init_schema(&tmp_conn)
        .map_err(|e| crate::core::error::CoreError::Database(format!(
            "恢复数据库 schema 初始化失败: {e}")))?;
    apply_migrations(
        &tmp_conn,
    ...
    // 5. 验证行数
    if let Some(cb) = on_progress { cb("验证数据...", 80); }
    ...
    // 8. 清理临时文件
    let _ = std::fs::remove_file(&tmp_db);
    if let Some(cb) = on_progress { cb("完成", 100); }
