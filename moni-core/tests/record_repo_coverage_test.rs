...
fn test_list_by_date_range_same_second() {
    ...
    record_repo::insert(&conn, 100, RecordType::Expense, cat_id, "同秒测试", Some(ts)).unwrap();
...
fn test_list_by_date_range_cross_year() {
    ...
    record_repo::insert(&conn, 100, RecordType::Expense, cat_id, "跨年-旧", Some(dec_31)).unwrap();
    record_repo::insert(&conn, 200, RecordType::Expense, cat_id, "跨年-新", Some(jan_01)).unwrap();
